...
sptr<tex::Font> Font_win32::deriveFont(int style) const {
  int s = convertStyle(style);
...
bool Font_win32::operator==(const tex::Font& ft) const {
  const Font_win32& f = static_cast<const Font_win32&>(ft);
  return _typeface.get() == f._typeface.get() && _size == f._size;
}

bool Font_win32::operator!=(const tex::Font& f) const {
  return !(*this == f);
}

tex::Font* tex::Font::create(const string& file, float s) {
  return new Font_win32(file, s);
}

sptr<tex::Font> tex::Font::_create(const string& name, int style, float size) {
...
const tex::Font* Graphics2D_win32::_defaultFont = nullptr;
...
const tex::Font* Graphics2D_win32::getFont() const {
  return _font;
}
